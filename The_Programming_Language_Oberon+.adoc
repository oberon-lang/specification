...
| SHORTINT  |  2 byte | the integers between MIN(SHORTINT) and MAX(SHORTINT)
| INTEGER   |  4 byte | the integers between MIN(INTEGER) and MAX(INTEGER)
| LONGINT   |  8 byte | the integers between MIN(LONGINT) and MAX(LONGINT)
...
[NOTE]
Because of the limited bit precision of the LONGREAL mantissa (which is 52 bits in IEEE 754 double precision representation), a LONGREAL does not fully include LONGINT. Similarly REAL does not include the full range of INTEGER. To convert a LONGINT to a LONGREAL or an INTEGER to a REAL the FLT() built-in function should be used.

A compiler is free to support other type inclusion relations in addition to the ones specified herein, but shall at least issue a warning if in a given operation information could be lost. A compiler shall at least support the Oberon 90 and Oberon-2 type inclusion relations in this way.

[NOTE]
Oberon 90 and Oberon-2 specify the following type inclusion relations which are not compatible with the ones specified herein: LONGREAL >= REAL >= LONGINT >= INTEGER >= SHORTINT.
...
At least array lengths between 1 and MAX(INTEGER) are supported, for both constant and variable lengths.
...
The type T of the case expression (case variable) may also be a record or pointer type. Then each case consists of exactly one case label which must be an extension of T, and in the statements S~i~ labelled by T~i~, the case variable is considered as of type T~i~. If the case variable is of POINTER type, then one case label can also be NIL. The evaluation order corresponds to the case label order; the first statement sequence is executed whose case label meets the condition.
...
|FLT(x)      |x: INTEGER or LONGINT|REAL or LONGREAL|Convert integer to real type; result is LONGREAL if x was LONGINT, else REAL, accepting potential loss of information
