...
Furthermore, enumeration types (known as _scalar types_ in Pascal <<Wi73>>), type-bound procedure types and explicit bit operations have been added to the language. IN can be used instead of VAR for constant variable parameters. The length of array type local variable can be determined at runtime. The foreign function interface (FFI) is a regular feature of the language.
